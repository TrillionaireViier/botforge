...
const SystemHealth = () => {
  const services = [
    { name: 'Main Database (PostgreSQL)', status: 'Operational', uptime: '99.99%', latency: '24ms', icon: <Database /> },
    { name: 'Trading Engine Core', status: 'Operational', uptime: '99.95%', latency: '12ms', icon: <Server /> },
    { name: 'Binance API WebSocket', status: 'Degraded', uptime: '98.50%', latency: '450ms', icon: <Cloud /> },
    { name: 'Authentication Service', status: 'Operational', uptime: '100%', latency: '18ms', icon: <ShieldCheck /> },
  ];
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="bg-white p-8 rounded-xl shadow-[4px_4px_0_0_rgba(0,0,0,1)] border-2 border-black flex flex-col md:flex-row items-center justify-between">
        <div className="flex flex-col mb-4 md:mb-0">
          ...
          <p className="text-gray-500 font-bold">Platform Uptime (30 days)</p>
        </div>
        ...
        <div className="flex flex-col text-center">
          ...
          <p className="text-gray-500 font-bold">Requests / Sec</p>
        </div>
        <div className="h-16 border-r-2 border-gray-200 hidden md:block"></div>
        <div className="flex flex-col text-right">
          <h2 className="text-2xl font-black text-gray-900">14ms</h2>
          <p className="text-gray-500 font-bold">Avg Response Time</p>
        </div>
      </div>
          ...
          {services.map((service, i) => (
            <div key={i} className="flex flex-col md:flex-row justify-between items-start md:items-center p-4 border-2 border-gray-100 rounded-lg">
              <div className="flex items-center space-x-4 mb-4 md:mb-0">
                <div className={`p-3 rounded-lg border-2 border-black ${service.status === 'Operational' ? 'bg-green-100 text-green-600' : 'bg-yellow-100 text-yellow-600'}`}>
                  {service.icon}
                </div>
                <div>
              ...
              <div className="flex items-center space-x-6 w-full md:w-auto">
                ...
                <div className={`px-4 py-2 rounded-md font-bold text-sm border-2 border-black flex-1 text-center ${service.status === 'Operational' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800 flex items-center justify-center'}`}>
