  ...
  useEffect(() => {
    const fetchTrades = async () => {
      try {
        ...
        console.error("Не удалось загрузить сделки администратора", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold text-gray-900">Журнал сделок платформы</h1>
        <button className="px-4 py-2 bg-black text-white rounded-md font-bold flex items-center hover:bg-gray-800 transition-colors">
          <DownloadCloud className="w-4 h-4 mr-2" /> Экспорт логов
        </button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-xl shadow-[4px_4px_0_0_rgba(0,0,0,1)] border-2 border-black">
          <p className="text-sm font-bold text-gray-500">Общий объем торгов</p>
          <div className="flex items-center space-x-4 mt-2">
            <h3 className="text-4xl font-black">${totalVolume.toLocaleString(undefined, { maximumFractionDigits: 2 })}</h3>
          </div>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-[4px_4px_0_0_rgba(0,0,0,1)] border-2 border-black">
          <p className="text-sm font-bold text-gray-500">Всего исполнено сделок</p>
          <div className="flex items-center space-x-4 mt-2">
            <h3 className="text-4xl font-black">{trades.length}</h3>
          </div>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-[4px_4px_0_0_rgba(0,0,0,1)] border-2 border-black">
          <p className="text-sm font-bold text-gray-500">Соотношение Покупки / Продажи</p>
          <div className="flex items-center space-x-4 mt-2">
      ...
      <div className="bg-white rounded-xl shadow-[4px_4px_0_0_rgba(0,0,0,1)] border-2 border-black overflow-hidden">
        <div className="p-6 border-b-2 border-black bg-gray-50">
          <h2 className="text-xl font-bold">Все сделки платформы</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-white border-b-2 border-black">
              <tr>
                <th className="px-6 py-3 text-left text-sm font-bold text-gray-900">ID Сделки</th>
                <th className="px-6 py-3 text-left text-sm font-bold text-gray-900">Пользователь</th>
                <th className="px-6 py-3 text-left text-sm font-bold text-gray-900">Бот</th>
                <th className="px-6 py-3 text-left text-sm font-bold text-gray-900">Пара / Тип</th>
                <th className="px-6 py-3 text-left text-sm font-bold text-gray-900">Сумма / Цена</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {trades.map((trade) => (
                <tr key={trade.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 font-mono font-bold text-gray-500">{String(trade.id).padStart(8, '0')}</td>
                  <td className="px-6 py-4 text-sm font-bold">{trade.user?.email || 'Неизвестно'}</td>
                  <td className="px-6 py-4 text-sm font-bold">{trade.bot?.name || 'Неизвестно'}</td>
                  ...
                  <td className={`px-6 py-4 font-bold ${trade.profit > 0 ? 'text-green-600' : trade.profit < 0 ? 'text-red-600' : ''}`}>
                    {trade.profit > 0 ? '+' : ''}${trade.profit > 0 ? trade.profit : '0.00'}
                  </td>
              ...
              {trades.length === 0 && !loading && (
                <tr>
                  <td colSpan="7" className="px-6 py-8 text-center text-gray-500 font-bold">Сделки в системе не найдены.</td>
                </tr>
              )}
            </tbody>
          </table>
